...
function handleConnection(world, client) {
  ...
  client.send({event: constants.SERVER_RESTARTED_EVENT, data: {cells: world.getAliveCells(world.layout), size: world.size}});
...
function handleClient(world, data, callback) {
  ...
  let newClientResponse = {event: constants.NEW_CLIENT_EVENT + '_RESPONSE', success: true, data: {cells: world.getAliveCells(world.layout), size: world.size, color: util.ipToColor(data.address), username: data.address}};
